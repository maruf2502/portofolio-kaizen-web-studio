...
ctaButtons.forEach(button => {
    button.addEventListener('click', (e) => {
        ...
        if (label.includes('konsultasi')) {
            ...
            if (label.includes('konsultasi gratis') || label.includes('mulai konsultasi')) {
                const waTemplate = [
...
document.querySelectorAll('.philosophy-card, .service-card, .package-card, .benefit-item, .timeline-item').forEach(el => {
    el.style.opacity = '0';
...
(() => {
    ...
    const resetWave = () => {
        ...
        letters.forEach((span) => span.style.transform = 'translateY(0px) scale(1)');
    };
...
(() => {
    ...
    const update = () => {
        ...
        hero.style.setProperty('--wave-opacity', (0.36 * (1 - progress)).toFixed(3));
        hero.style.setProperty('--hero-spot-opacity', (0.14 * (1 - progress * 0.85)).toFixed(3));

        if (heroVisual) {
            heroVisual.style.setProperty('--hero-scroll-y', `${visualOffset.toFixed(2)}px`);
        }

        if (heroContent) {
            heroContent.style.setProperty('--hero-content-translate', `${contentOffset.toFixed(2)}px`);
            heroContent.style.setProperty('--hero-content-opacity', `${(1 - progress * 0.4).toFixed(3)}`);
        }
    };
...
document.querySelectorAll('.philosophy-card, .service-card, .package-card, .benefit-item, .timeline-item').forEach((card) => {
    enableSpotlightTilt(card, { maxTilt: card.classList.contains('benefit-item') ? 6 : 8 });
});
...
document.querySelectorAll('[data-package-select]').forEach((btn) => {
    btn.addEventListener('click', () => {
        ...
        selectPackage(card, { scrollToContact: false });
        openPackageOrderModal({
            packageName: card?.dataset?.package || btn.dataset.packageSelect || 'Paket',
            packagePrice: card?.dataset?.price || ''
        });
    });
});
...
function openPackageOrderModal({ packageName, packagePrice } = {}) {
    ...
    if (!overlay || !title || !subtitle || !summaryValue || !form || !nameInput || !emailInput || !companyInput || !pkgNameInput || !pkgPriceInput) return;
    ...
    // Reset form state
    form.reset();
    [nameInput, emailInput, companyInput].forEach((el) => el.classList.remove('is-invalid'));
...
(() => {
    ...
    if (!overlay || !closeBtn || !form || !nameInput || !emailInput || !companyInput) return;
    ...
    form.addEventListener('submit', (e) => {
        ...
        const pkgName = sanitizeWhatsAppSingleLine(document.getElementById('orderPackageName')?.value || selectedPackageForOrder.name || 'Paket');
        const pkgPrice = sanitizeWhatsAppSingleLine(document.getElementById('orderPackagePrice')?.value || selectedPackageForOrder.price || '');
